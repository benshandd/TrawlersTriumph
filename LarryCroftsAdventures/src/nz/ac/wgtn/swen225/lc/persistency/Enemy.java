...
public class Enemy {
    ...
    public Enemy move(Tile[][] grid) {
        ...
        if (secondsElapsed >= 3) {
            ...
            if (isValidMove(grid, newX, newY)) {
                ...
                // Not a valid move?, generate a different direction and create a new Enemy object
    ...
    /**
     * Calculates the movement direction for the enemy to approach the player character (Chap).
     *
    ...
    private boolean isValidMove(Tile[][] grid, int x, int y) {
        // Check if the coordinates are within the bounds of the grid and the destination is a Free tile
